...
// Entities with no live source yet. `trainings` is the LearningLab training
// business module (internal, external and government/public training).
const SEED_ENTITIES = [
  'deals', 'visits', 'campaigns', 'proposals', 'payrollClients', 'vendors', 'targets', 'dailyReports', 'trainings'
];
const store = {};
SEED_ENTITIES.forEach((n) => { store[n] = []; });
store.trainings = TRAININGS.slice();
