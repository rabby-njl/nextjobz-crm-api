...
// Entities with no live source yet — empty (no demo data). In-memory CRUD only.
const SEED_ENTITIES = [
...
function nextId(entity, list) {
  const prefixes = {
    ...
    payrollClients: 'PAY', vendors: 'VEND', targets: 'TGT', dailyReports: 'REPORT'
  };
...
async function handle(req, res) {
  ...
  // Auth endpoints (public login + session check)
  if (parts[1] === 'auth') {
